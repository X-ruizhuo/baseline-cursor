...
class LTSKAdapterLite(nn.Module):
    """Memory-conscious feature adapter for fusion with continual re-id.

    The adapter keeps the Bi-C2R interface (feature in, feature out), but uses
    a small residual projector plus a tiny prototype bank and gate. This keeps
    parameter count and activation memory low while still providing a place to
    inject LTSKC++-style knowledge organization.
    """

    def __init__(self, in_planes=2048, hidden_planes=256, num_prototype=8):
        super(LTSKAdapterLite, self).__init__()
        hidden_planes = max(64, min(hidden_planes, in_planes // 8))
        self.project = nn.Sequential(
            nn.Linear(in_planes, hidden_planes, bias=False),
            nn.BatchNorm1d(hidden_planes, eps=2e-05, momentum=0.9),
            nn.PReLU(hidden_planes),
            nn.Linear(hidden_planes, in_planes, bias=False),
            nn.BatchNorm1d(in_planes, eps=2e-05, momentum=0.9),
        )
        self.gate = nn.Sequential(
            nn.Linear(in_planes, hidden_planes, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_planes, 1, bias=True),
            nn.Sigmoid(),
        )
        self.prototype = nn.Parameter(torch.randn(num_prototype, in_planes) * 0.02)
        self.prototype_gate = nn.Sequential(
            nn.Linear(in_planes, hidden_planes, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_planes, num_prototype, bias=True),
            nn.Softmax(dim=-1),
        )
        self.norm = nn.BatchNorm1d(in_planes, eps=2e-05, momentum=0.9)
        self.norm.bias.requires_grad_(False)
        nn.init.constant_(self.norm.weight, 1)
        nn.init.constant_(self.norm.bias, 0)
        self.project.apply(weights_init_kaiming)
        self.gate.apply(weights_init_kaiming)
        self.prototype_gate.apply(weights_init_kaiming)

    def forward(self, x):
        residual = x
        proj = self.project(x)
        gate = self.gate(x)
        proto_w = self.prototype_gate(x)
        proto = torch.matmul(proto_w, self.prototype)
        out = residual + gate * proj + (1.0 - gate) * proto
        out = self.norm(out)
        return out
